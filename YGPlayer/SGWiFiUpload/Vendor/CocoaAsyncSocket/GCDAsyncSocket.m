...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark - GCDAsyncSocketPreBuffer
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 ...
 *
 *  @param bufferPtr         读指针
 *  @param availableBytesPtr 有效剩余读取字节
 */
 ...
 *
 *  @param bufferPtr         写指针
 *  @param availableSpacePtr 有效的写空间
 */
...
**/
//是直接读到read packet's buffer中还是读到preBuffer
- (NSUInteger)readLengthForTermWithHint:(NSUInteger)bytesAvailable shouldPreBuffer:(BOOL*)shouldPreBufferPtr
{
    ...
    if (shouldPreBufferPtr) {
        ...
        //获取当前buffer已使用的大学
        NSUInteger buffUsed = startOffset + bytesDone;
        //判断当前剩余空间是否够用，够用就直接写用，否则就用preBuffer
...
- (NSUInteger)readLengthForTermWithPreBuffer:(GCDAsyncSocketPreBuffer*)preBuffer found:(BOOL*)foundPtr
{
    ...
    //     NSString *bufstring=[[NSString alloc ] initWithBytes:[preBuffer readBuffer] length:preBufferLength encoding:NSUTF8StringEncoding];
...
- (uint8_t*)currentReadOffset
{
    return (uint8_t*)[buffer mutableBytes] + startOffset + bytesDone;
}
@end

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -Write Packet
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//对6.0的适配，如果是6.0以下，则去retain release，6.0之后ARC也管理了GCD
        ...
        /*生成了一个socketQueue，这个queue是串行的，接下来我们看代码就会知道它贯穿于这个类的所有地方。所有对socket以及一些内部数据的相关操作，都需要在这个串行queue中进行。这样使得整个类没有加一个锁，就保证了整个类的线程安全。*/
        ...
        if (sq) {
            ...
            //没有的话创建一个，  名字为：GCDAsyncSocket,串行队列
        ...
        //可重入的概念有点类似递归，一个函数，如果能够并行进入该函数且不出现问题，则可为可重入，而GCD是一个对共享资源的访问进行串行化的队列，所以是不可重入的，需要寻找一些技巧去绕开这个限制；
...
{
    if (socketFD != SOCKET_NULL && (socketFD == socket6FD || socketFD == socket4FD)) {
        close(socketFD);
...
- (NSUInteger)copyPreBuffer2CurrentRead:(GCDAsyncReadPacket*)acurrentRead PreBuffer:(GCDAsyncSocketPreBuffer*)apreBuffer found:(BOOL*)foundPtr
{
    ...
    //如果当前读的数据界限不为空
    if (acurrentRead->term != nil) {
        // Read type #3 - read up to a terminator
        //直接读到界限
        bytesToCopy = [acurrentRead readLengthForTermWithPreBuffer:apreBuffer found:foundPtr];
    } else {
        // Read type #1 or #2
        //读取数据，读到指定长度或者数据包的长度为止
        bytesToCopy = [acurrentRead readLengthForNonTermWithHint:[apreBuffer availableBytes]];
    }
    LogVerbose(@"copying %lu bytes from preBuffer", (unsigned long)bytesToCopy);
    //把数据从preBuffer中移到currentRead上
    [self preBuffer2CurrentRead:acurrentRead PreBuffer:apreBuffer bytesToCopy:bytesToCopy];

    //    NSString* bufstring = [[NSString alloc] initWithBytes:readBuf length:bytesToCopy encoding:NSUTF8StringEncoding];
    return bytesToCopy;
}

#pragma mark -
- (void)preBuffer2CurrentRead:(GCDAsyncReadPacket*)acurrentRead PreBuffer:(GCDAsyncSocketPreBuffer*)apreBuffer bytesToCopy:(NSUInteger)bytesToCopy
{
    // Ensure there's room on the read packet's buffer
    //确保有这么大的空间
    [acurrentRead ensureCapacityForAdditionalDataOfLength:bytesToCopy];

    // Copy bytes from prebuffer into read buffer
    uint8_t* readBuf = [acurrentRead currentReadOffset];

    //把数据从preBuffer中移到currentRead上
    memcpy(readBuf, [apreBuffer readBuffer], bytesToCopy);
}

#pragma mark -读取数据核心方法
- (void)doReadData
{
    LogTrace();
    ...
    if ([preBuffer availableBytes] > 0) {
        ...
        // 把数据从preBuffer中移到currentRead上
        NSUInteger bytesToCopy = [self copyPreBuffer2CurrentRead:currentRead PreBuffer:preBuffer found:&done];
        ...
        if (flags & kSocketSecure) {
            ...
            if ([self usingCFStreamForTLS]) {
                ...
                //不用prebuffer
                    ...
                    buffer = [currentRead currentReadOffset];
                }
        ...
        if (bytesRead > 0) {
            ...
            //没边界，没给定长度（无法判断当前包结尾）
                ...
                if (readIntoPreBuffer) {
                    ...
                    [self preBuffer2CurrentRead:currentRead PreBuffer:preBuffer bytesToCopy:bytesRead];
...
//初始化读的超时,此方法到timeout小于0，则啥也不做，也就是说延时无效
