...
#if HTTPASYNC_FILE
#import "HTTPAsyncFileResponse2.h"
#else
...
- (id)initWithAsyncSocket:(GCDAsyncSocket*)newSocket configuration:(HTTPConfig*)aConfig
{
    ...
    if ((self = [super init])) {
        ...
        if (aConfig.queue) {
            ...
            HTTPLogTrace2(@"1 connectionQueue:%p", connectionQueue);
#if !OS_OBJECT_USE_OBJC
            dispatch_retain(connectionQueue);
#endif
        } else {
            connectionQueue = dispatch_queue_create("com.ftx.HTTPConnection", NULL);
            HTTPLogTrace2(@"2 connectionQueue:%p", connectionQueue);
        }
...
- (NSString*)requestURI
{
    ...
    //解码还原中文字符
    return [[[request url] relativeString] stringByRemovingPercentEncoding];
}
...
#pragma clang diagnostic pop
    ...
    LOG4(@"NSThread currentThread:%@ 请求命令:%@", [NSThread currentThread], str);
#endif
    ...
    HTTPLogInfo(@"isWebSocket 1 uri:%@", uri);

    // Check for WebSocket request
    if ([WebSocket isWebSocketRequest:request]) {
        HTTPLogTrace2(@"isWebSocket yes 2");
        ...
        if (ws == nil) {
            ...
            if ([asyncSocket delegate] == (id)self) {
...
- (void)sendResponseHeadersAndBody
{
    ...
    if ([[request method] isEqualToString:@"HEAD"] || isZeroLengthResponse) {
        ...
        LOG41(@"NSThread currentThread:%@  将响应头部信息写入Socket:%@", [NSThread currentThread], str);
#endif
...
- (void)continueSendingStandardResponseBody
{
    ...
    HTTPLogInfo(@"%@ writeQueueSize:%lu", THIS_FILE, (unsigned long)writeQueueSize);
    if (writeQueueSize >= READ_CHUNKSIZE)
        return;

    NSUInteger available = READ_CHUNKSIZE - writeQueueSize;
    HTTPLogInfo(@"%@ available:%lu", THIS_FILE, (unsigned long)available);
...
- (void)continueSendingSingleRangeResponseBody
{
    ...
    NSUInteger writeQueueSize = [self writeQueueSize];
    //HTTPLogInfo(@"%@ writeQueueSize:%lu", THIS_FILE, writeQueueSize);
    if (writeQueueSize >= READ_CHUNKSIZE)
        return;
    ...
    if (bytesLeft > 0) {
        ...
        if ([data length] > 0) {
            ...
            HTTPLogInfo(@"@_@:%@ writeData offset:%llu bytesToRead:%lu", THIS_FILE, offset, (unsigned long)bytesToRead);
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                ...
                [[NSNotificationCenter defaultCenter]
                    postNotificationName:EADSessionDataReceivedCountNotification
                                  object:[NSNumber numberWithUnsignedLongLong:[httpResponse offset]]];
            });

#endif
            //debugHexLog((unsigned char*)data.bytes, 20, @"写入socket data:");
            //将数据写入socket
...
- (NSString*)filePathForURI:(NSString*)path allowDirectory:(BOOL)allowDirectory
{
    ...
    NSString* relativePath = [[NSURL URLWithString:[path stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]] relativeToURL:docRoot] relativePath];
...
**/
#pragma mark - Generally better for larger files 生成大文件数据
- (NSObject<HTTPResponse>*)httpResponseForMethod:(NSString*)method URI:(NSString*)path
{
    HTTPLogTrace();
#if HTTPASYNC_FILE
    HTTPLogInfo(@"filePath:%@", path);
    return [[HTTPAsyncFileResponse2 alloc] initWithFilePath:path forConnection:self];
#else
...
**/
#pragma mark - sock 读取数据
- (void)socket:(GCDAsyncSocket*)sock didReadData:(NSData*)data withTag:(long)tag
{
#if LOG_ASYNC
    NSString* str = [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding];
    LOG4(@"NSThread currentThread:%@ HttPConnection didReadData 拿到数据:%@", [NSThread currentThread], str);
#endif
...
- (void)socket:(GCDAsyncSocket*)sock didWriteDataWithTag:(long)tag
{
    ...
    if (tag == HTTP_PARTIAL_RESPONSE_BODY) {
        ...
        HTTPLogInfo(@"shouldDie 000:%ld", tag);
    ...
    if (doneSendingResponse) {
        ...
        if (tag == HTTP_FINAL_RESPONSE) {
            ...
            HTTPLogInfo(@"shouldDie 111:%ld", tag);
            if ([self shouldDie]) {
                // Cleanup after the last request
