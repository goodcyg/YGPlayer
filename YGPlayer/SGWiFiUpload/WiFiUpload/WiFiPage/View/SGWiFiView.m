...
- (NSAttributedString*)descTextWithAddress:(NSString*)address
{
    ...
    NSMutableAttributedString* textBegin = [[NSMutableAttributedString alloc] initWithString:@"1. Please make sure your iPhone and PC is connected to the same WiFi.\n\n2. Open the URL below in a browser:\n\n     "];

    [textBegin appendAttributedString:text];

    NSMutableAttributedString* textEnd = [[NSMutableAttributedString alloc] initWithString:@"\n\n3. When click the submit button on the website, please wait until the page finish loading."];
    [textBegin appendAttributedString:textEnd];
    return textBegin;
...
- (void)commonInit
{
    ...
    descLabel.attributedText = [self descTextWithAddress:@"Fetching..."];
...
{
    self.descLabel.attributedText = [self descTextWithAddress:address];
}
