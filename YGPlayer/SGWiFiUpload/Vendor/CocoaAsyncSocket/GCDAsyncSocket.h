...
/*Socket就是一种特殊的文件。它是一个连接了两个用户的文件，任何一个用户向Socket里写数据，
 另一个用户都能看得到，不管这两个用户分布在世界上相距多么遥远的角落，感觉就像坐在一起传纸条一样。
 这么讲Socket应该更容易理解吧？这种抽象是非常重要的，因为它屏蔽了更底层的东西，我就想写个程序发送下数据，
 为什么要关系物理层怎么传输呢，对吧。
